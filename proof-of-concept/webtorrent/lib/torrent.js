...
const debugAxon = require('debug')('axon:torrent')
...
class Torrent extends EventEmitter {
  constructor (torrentId, client, opts) {
    ...
    this.initAxon({
      ethereumWallet: this.client.ethereumWallet
    }).then(node => { this.axonNode = node })
  }

  async initAxon({ ethereumWallet }) {
    const { Node } = await import('axonproto-lib')
    // console.log((await import('axonproto-lib')).Node)
    const node = new Node(ethereumWallet)
    node.start()
    return node
  }
  ...
  getFileModtimes (cb) {
    ...
    parallelLimit(this.files.map((file, index) => cb => {
      console.log(this.path, file.path)
  ...
  _registerPeer (newPeer) {
    ...
    // newPeer.on('wire'
    newPeer.once('wire-ready', (wire) => {
      this._debugAxon('debug got a new wire')

      wire.on('piece-upload', (pieceInfo) => {
        this._debugAxon('upload piece')
        this.axonNode.upload([
          {
            key: "upload",
            // TODO:axon
            eth_wallet: this.axonNode.wallet.address,
            torrent: this.infoHash,
            from_peer: this.client.peerId,
            to_peer: wire.peerId,
            piece: pieceInfo.index,
            time: +new Date(),
          }
        ])
      })
      ...
      wire.on('piece', (index, offset, buffer) => {
        this._debugAxon('download-piece')
        if (!this.axonNode) return
        this.axonNode.download([
          {
            key: 'download',
            // TODO:axon
            eth_wallet: this.axonNode.wallet.address,
            torrent: this.infoHash,
            to_peer: this.client.peerId,
            from_peer: wire.peerId,
            piece: index,
            time: +new Date(),
          }
        ])
    ...
    wire.request(index, chunkOffset, chunkLength, function onChunk (err, chunk) {
      ...
      sha1(buf, hash => {
        ...
        if (hash === self._hashes[index]) {
          ...
          self.emit('piece-verified', {index, chunkOffset, chunkLength, wire})

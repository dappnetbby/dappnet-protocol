...
# case: anew
if [ $type == "anew" ]; then
    echo "Deploying a clean deployment"
    rm manifest.json
fi
...
if [ $type == "anew" ]; then
    ...
    forge script --gas-estimate-multiplier 200 --rpc-url "http://localhost:8545" --broadcast --sender 0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266 --private-key $PRIVATE_KEY script/Init.s.sol --tc InitScript
fi